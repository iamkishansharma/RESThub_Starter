        ...
        func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
            ...
            let starAction = UIContextualAction(style: .normal, title: "Star") { (action, view, completion) in
                ...
                DataServices.shared.starUnstargist(id: "\(currentGist.id)", star: true){
                    (success) in
                    DispatchQueue.main.async {
                        if success{
            ...
            let unstarAction = UIContextualAction(style: .normal, title: "Unstar") { (action, view, completion) in
                ...
                DataServices.shared.starUnstargist(id: "\(currentGist.id)", star: false){
                    (success) in
                    DispatchQueue.main.async {
                        if success{
                            self.showResultAlert(title: "Success :)", message: "Gist successfully Un-starred!")
                        }else{
                            self.showResultAlert(title: "Oops :(", message: "Something went wrong...")
                        }
                    }
